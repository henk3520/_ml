...
class Player:
    ...
    def chooseAction(self, positions, current_board, symbol):
        ...
        for p in positions:
            ...
            next_board[p] = symbol
            if self.is_winning_move(next_board, symbol):
                return p

        # 檢查是否需要阻止對手贏得比賽
        for p in positions:
            next_board = current_board.copy()
            next_board[p] = opponent_symbol
            if self.is_winning_move(next_board, opponent_symbol):
                return p
        ...
        if np.random.uniform(0, 1) <= self.exp_rate:
            ...
            for p in positions:
                ...
                if value > value_max:
                    value_max = value
                    actions_with_max_value = [p]
                elif value == value_max:
                    actions_with_max_value.append(p)

            if actions_with_max_value:
                action = np.random.choice(len(actions_with_max_value))
                action = actions_with_max_value[action]
            else:
                action = positions[0] if positions else None
        return action

    def is_winning_move(self, board, symbol):
        # 檢查是否為勝利的行動
        # 檢查水平、垂直和對角線方向
        for c in range(BOARD_COLS - 3):
            for r in range(BOARD_ROWS):
                if sum(board[r, c:c + 4]) == 4 * symbol:
                    return True
        for c in range(BOARD_COLS):
            for r in range(BOARD_ROWS - 3):
                if sum(board[r:r + 4, c]) == 4 * symbol:
                    return True
        for c in range(BOARD_COLS - 3):
            for r in range(BOARD_ROWS - 3):
                if sum([board[r + i, c + i] for i in range(4)]) == 4 * symbol:
                    return True
                if sum([board[r + 3 - i, c + i] for i in range(4)]) == 4 * symbol:
                    return True
        return False
